...
class TestCursorAdapter:
    ...
    async def test_sync_agents(self, temp_config_dir):
        ...
        with patch("pathlib.Path.cwd") as mock_cwd:
            ...
            mock_metadata = MagicMock()
            mock_metadata.name = "test-agent"
            mock_metadata.category = "engineering"
            mock_agent.metadata = mock_metadata
        ...
        # Check if rule file was created (now using .cursorrules format for project rules)
        rule_file = temp_config_dir / "test-agent.cursorrules"
        assert rule_file.exists()

        content = rule_file.read_text()
        # For project-level rules, content is written as-is
        assert content == "Test instructions for code review"
...
class TestAdapterIntegration:
    ...
    async def test_migration_between_adapters(self, temp_config_dir):
        ...
        # Verify rule file was created (now using .cursorrules format)
        rule_file = temp_config_dir / "cursor" / "test-agent.cursorrules"
        assert rule_file.exists()
